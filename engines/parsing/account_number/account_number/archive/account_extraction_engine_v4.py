"""
Account Number Extraction Engine v4.0
...
def _extract_robinson_waste(text: str) -> Optional[str]:
    ...
    Examples: 55779.64, 55779.152, 2246
    """
...
def _extract_meridian_waste(text: str) -> Optional[str]:
    """Format: NN-NNNNNNN(N) N after Account No.
    V3 FIX: Accept 7-8 digits after hyphen (some accounts have 8)
    Examples: 01-1276236 4, 70-0143542 2, 01-12756754
    """
    match = re.search(r'Account\s*No\.?\s*(\d{2}-\d{7,8}\s*\d?)', text, re.IGNORECASE)
    return match.group(1).strip() if match else None
...
def _extract_athens_services(text: str) -> Optional[str]:
    """Format: Alphanumeric (2M, CE, TC prefixes)
    V3 FIX: More flexible pattern matching for header structure
    Examples: 2M0010827, 2M0011054, CE0019867, TC0003456
    """
    lines = text.split('\\n')
            ...
            for j in range(i, min(i+5, len(lines))):
                if 'NUMBER' in lines[j].upper() or j > i:
                    for k in range(j, min(j+4, len(lines))):
                        val = lines[k].strip()
                        # Match 2M0010827, CE0019867, TC0003456
                        if re.match(r'^[A-Z]{1,2}\d{6,10}$', val):
                            return val
                        # Also match with hyphen like 2M-0010827
                        if re.match(r'^[A-Z]{1,2}[0-9\-]{6,10}$', val):
                            return val
    return None
