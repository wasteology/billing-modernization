    ...
    for i, line in enumerate(lines):
        if re.search(r'Customer\s*ID', line, re.I):
            for j in range(i + 1, min(i + 5, len(lines))):
                ...
                if m2:
                    return m2.group(1)
    # WGY code (WM vendor account): WGY20070UP, WGY40213AA
    m = re.search(r'(WGY\w{5,10})', normalized)
    if m:
        return m.group(1)
...
_NO_ACCOUNT_VENDORS_V93D = [
    ...
    'Walker Lake Disposal',      # QBO invoices, PO# only (W-711-6 format)
    'Mills Brothers',            # No account labels in any samples
    'Pacific Waste',             # No account labels
    'Greif',                     # Industrial packaging, no account field
    'CHOCTAW COUNTY WASTE SERVICES',  # Municipal, no account
    'Honolulu Disposal',         # No account labels
    'CAROLINA COMMERCIAL & INDUSTRIAL REFURISHMENT LLC',  # No account
    'Cyclyx International LLC',  # Recycling, no account labels
    'Sutherlin Sanitary',        # No account labels
    'ALLIANCE INDUSTRIAL CORP',  # Industrial, no account
    'automotive recyclers association',  # Association, no account
    'PRIDE Disposal',            # No account labels
    ...
    'Suburban Disposal',         # No account labels
    ...
    'Robinson Waste',            # Work Order # only, no customer account
    'American Recycling',        # Scale tickets with "Cust IDSTANDARD-1" text IDs, not accounts
    # From Step 3 review grind — round 2 (2026-03-11)
    'Solid Waste Authority',     # Weigh/charge tickets (ticket# only, no customer account)
    'Checksammy',                # Payment receipts only (Invoice # but no customer account)
    'AT Disposal',               # Roll-off dumpster rental (no customer account field)
    'Disposal Management',       # Misdetected as Disposal Mgmt — actually Atlas rate letters
    'Universal Waste',           # Contamination audits and certificates, not invoices
...
V93D_EXPLICIT_OVERRIDES = {
    ...
    *_NO_ACCOUNT_VENDORS_V93D,
    # Intentional has_account changes from mar2026
    'Jettison Environmental',
    'Derby City Environmental',
    'Mark Dunning',
    'A1 Waste',
